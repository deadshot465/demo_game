...
pub enum CameraType {
    Watch(Vec3A), Directional(Vec3A), Chase(Vec3A), TPS(Vec3A, f32), FPS(Vec3A, f32)
}
...
impl Camera {
    ...
    pub fn get_view_matrix(&self) -> Mat4 {
        Mat4::look_at_rh(Vec3::from(self.position), Vec3::from(self.target), Vec3::new(0.0, -1.0, 0.0))
    }
