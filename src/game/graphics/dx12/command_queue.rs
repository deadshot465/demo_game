...
use winapi::um::d3d12::{
    ID3D12CommandAllocator, ID3D12CommandList, ID3D12CommandQueue, ID3D12Device2,
    ID3D12GraphicsCommandList1, D3D12_COMMAND_LIST_TYPE_DIRECT, D3D12_COMMAND_QUEUE_DESC,
...
impl CommandQueue {
    ...
    unsafe fn create_command_queue(device: &ComPtr<ID3D12Device2>) -> ComPtr<ID3D12CommandQueue> {
        ...
        let res = device.CreateCommandQueue(
            &desc as *const _,
            &ID3D12CommandQueue::uuidof() as REFIID,
            &mut ptr as *mut _,
        );
    ...
    pub unsafe fn create_command_allocators(
        device: &ComPtr<ID3D12Device2>,
        buffer_count: UINT,
    ) -> Vec<ComPtr<ID3D12CommandAllocator>> {
    ...
    ) -> ComPtr<ID3D12GraphicsCommandList1> {
        let mut ptr = get_nullptr();
